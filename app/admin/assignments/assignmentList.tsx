...
import { Assignment, AssignmentStatus, StatusFilter } from "./types";
import { fmtDate, initials2, isOverdue } from "./utils";
import { statusBadge, statusDot } from "./colors";
...
export default function AssignmentList() {
  ...
  const handleAssigned = useCallback((newOnes: Assignment[]) => {
    setAssignments((p) => [...newOnes, ...p]);
  }, []);

  const handleUpdated = useCallback((a: Assignment) => {
    setAssignments((p) => p.map((x) => x.assignmentId === a.assignmentId ? a : x));
  }, []);

  const handleDeleted = useCallback((id: string) => {
    setAssignments((p) => p.filter((x) => x.assignmentId !== id));
  }, []);
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            ...
            {loading
              ...
              : filtered.map((a) => {
                  ...
                  const effectiveStatus = overdueFlag && a.status !== "Completed" ? "Overdue" : a.status;
                  return (
                    <div
                      key={a.assignmentId}
